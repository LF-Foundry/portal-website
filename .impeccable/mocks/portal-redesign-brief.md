...
- Video states: poster/initial frame, loading, playing, paused, ended/loop, unsupported/error, and reduced-motion. Controls remain keyboard reachable; meaningful audio requires captions or a transcript.
...
- Type character: a bold, broad grotesk for the thesis; a compact UI grotesk for navigation, labels, and body copy. Use the existing Geist variable family for native desktop precision, with scale and weight carrying the distinction.
...
| Showcase film | Real 20.17-second 3840x1608 Portal video, uncropped; lazy media loading and resilient fallback | Existing video asset via semantic `video` |
...
| Feature proof | Four concise product-specific behaviors paced as alternating quiet and dense passages | Semantic HTML/CSS with small authored diagrams |
| Download surface | Two operating-system choices, latest-release links, and installation-guide links | Reusable platform cards |
...
- The supplied `.mov` is 35.6 MB and should be replaced by optimized web sources plus a poster when conversion tooling is available. The component must make that replacement mechanical.
- The video frame content and audio were not decodable in this session; poster selection and caption requirements must be verified from the final encoded media rather than guessed.
